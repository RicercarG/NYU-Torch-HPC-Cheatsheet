...
if [ -d $env_dir ]; then
  ...
  select opt in "${options[@]}"; do
    ...
    "start singularity env in read only mode")
      ...
      singularity exec --fakeroot --nv --bind $data_dir --overlay $overlay:ro $singularity_file /bin/bash
      exit 1
      break
      ;;
    "start singularity env in read and write mode")
      ...
      echo "Always remember to activate your conda environment by typing: source /ext3/env.sh"
      singularity exec --fakeroot --nv --bind $data_dir --overlay $overlay:rw $singularity_file /bin/bash
      exit 1
      break
      ;;
...
if [ $install_new_env -eq 1 ]; then
  ...
  options=("cuda 11.6" "cuda 11.8" "cuda 12.1" "cuda 12.2" "cuda 12.3" "cuda 12.6" "cuda 12.8" "cuda 13.0" "Custom CUDA version (build from Docker Hub)")
  select opt in "${options[@]}"; do
    case $opt in
    "cuda 11.6")
      singularity_file="cuda11.6.2-cudnn8.14.1.sqf"
    ...
    "cuda 12.1")
      ...
      ;;
    "cuda 12.2")
      singularity_file="cuda12.2.2-cudnn8.9.4-devel-ubuntu22.04.3.sif"
      break
      ;;
    "cuda 12.3")
      singularity_file="cuda12.3.2-cudnn9.0.0-ubuntu-22.04.4.sif"
      break
      ;;
    "cuda 12.6")
      singularity_file="cuda12.6.3-cudnn9.5.1-ubuntu22.04.5.sif"
      break
      ;;
    "cuda 12.8")
      singularity_file="cuda12.8.1-cudnn9.8.0-ubuntu24.04.2.sif"
      break
      ;;
    "cuda 13.0")
      singularity_file="cuda13.0.1-cudnn9.13.0-ubuntu-24.04.3.sif"
      break
      ;;
    "Custom CUDA version (build from Docker Hub)")
      read -rp "Enter CUDA major.minor (e.g. 12.4): " cuda_ver
      if [[ ! $cuda_ver =~ ^[0-9]+\.[0-9]+$ ]]; then
        echo "Invalid CUDA version format. Exiting."
        exit 1
      fi
  ...
  if [ "$CUSTOM_BUILD" -eq 0 ]; then
    ...
    if [ ! -f "$env_dir/$singularity_file" ]; then
      cp -rp "/share/apps/images/$singularity_file" "$env_dir/"
    fi
  else
  ...
  cp -rp /share/apps/overlay-fs-ext3/$overlay.gz $env_dir
...
if [ $install_new_conda -eq 1 ]; then
  ...
  if [ "$CUSTOM_BUILD" -eq 1 ]; then
    ...
    cd "$env_dir"
    singularity exec --fakeroot --nv --bind $(pwd):/mnt --overlay $overlay:rw $singularity_file /bin/bash -c "
...
EOF
    ...
    singularity exec --fakeroot --nv --bind $data_dir --overlay $overlay:rw $singularity_file /bin/bash -c "
...
if [ $used_for_ood -eq 1 ]; then
  ...
  cp -R /share/apps/kernel_template ./$folder_name # this should be the name of your Singularity env
  cd ./$folder_name
  old_word="UPDATE_TO_OVERLAY.EXT3_FILEPATH"
  new_word="$overlay"
  sed -i "s#$old_word#$new_word#g" "./python"
  old_word="/share/apps/images/cuda12.8.1-cudnn9.8.0-ubuntu24.04.2.sif"
  new_word="$singularity_file"
  sed -i "s#$old_word#$new_word#g" "./python"

  old_word="UPDATE_TO_KERNEL_PYTHON_FILE_PATH"
  new_word="/home/$USER/.local/share/jupyter/kernels/$folder_name/python"
  sed -i "s#$old_word#$new_word#g" "./kernel.json"
  old_word="UPDATE_TO_KERNEL_NAME"
...
echo -e "singularity exec --fakeroot --nv --bind $data_dir --overlay $overlay:rw $singularity_file /bin/bash${NC}"
